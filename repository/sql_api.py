...
def insert_rows_into_processed_data1(connector: StoreConnector, dataframe: DataFrame):
    ...
    if len(files_list) > 0:
        for row in rows:
            connector.execute(f'INSERT INTO processed_data1 (genres, title_movie, production_countries, Release_year, tagline, source_file) VALUES ( \'{row["genres"]}\', \'{row["title_movie"]}\', \'{row["production_countries"]}\', \'{row["Release_year"]}\', \'{row["tagline"]}\', {last_file_id})')
